...
const GuestRatingBadge: React.FC<GuestRatingBadgeProps> = ({ ratingRange, hotelCount }) => {
    ...
    return (
        <div data-cy={'rating-badge'} className="flex items-center justify-between flex-1">
            <span data-cy={'guest-rate'} className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${getBadgeStyles(ratingRange)}`}>
            ...
            <span data-cy={'guest-rating-count'} className="text-xs text-gray-500 ml-2">({hotelCount})</span>
        </div>
    );
};
