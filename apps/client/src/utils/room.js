...
export function parseRoomJson(json) {
    ...
    if (smokingPolicy.includes("Non-Smoking"))
        extracted.smokingPolicy = smokingPolicy;
    const displayFields = json.roomAdditionalInfo?.displayFields;
    const extractListAndNote = (html) => {
        const $ = cheerio.load(html);
        const items = [];
        $('li').each((_, el) => {
...
export function getLowestRoomPrice(rooms) {
