                ...
                cy.get("[data-cy=rating-text]").each(($span) => {
                    cy.wrap($span).invoke("text").then((text) => {
                        ...
                        if (guestRatingMap.has(rating)) {
                            guestRatingMap.set(rating, guestRatingMap.get(rating) + 1);
                        }
                        else {
                            guestRatingMap.set(rating, 1);
                        }
                    });
                });
            });
...
//# sourceMappingURL=GuestRatingFilter.cy.js.map